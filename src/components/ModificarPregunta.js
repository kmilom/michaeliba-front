...
export default function ModificarPregunta (props){
    
    const [show, setShow] = useState(false);
    const handleClose = () => setShow(false);
    const handleShow = () => setShow(true);
    
    const estadoInicialFormPregunta = vectorPreguntas.find(p=> p.id === props.props);
    ...
    const onSubmitForm = e => {
        e.preventDefault();
        let idPreguntaEditar = vectorPreguntas.findIndex(p => p.id === props.props);
        vectorPreguntas[idPreguntaEditar] = estadoForm;
